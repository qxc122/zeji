...
- (void) setupBadgeViewWithString:(NSString *)string
{
    ...
    self.badgeTextColor = [UIColor whiteColor];
    [badgeLabel setFont:[UIFont systemFontOfSize:FONTSIZE_BADGE]];
    CGSize badgeSize = [badgeLabel sizeThatFits:CGSizeMake(320, FLT_MAX)];
    
    badgeSize.width = badgeSize.width < MINIWIDTHANDHEIGHTVALUE ? MINIWIDTHANDHEIGHT : badgeSize.width + 5;
    ...
    [badgeLabel setFrame:CGRectMake(self.bounds.size.width - 10 + horizontal, -(badgeSize.height / 2) - 10 + vertical, badgeSize.width,  badgeSize.width > MINIWIDTHANDHEIGHT ? badgeSize.height : badgeSize.width)];
    [self setupBadgeStyle];
...
- (void) setupBadgeStyle
{
    ...
    badgeLabel.layer.cornerRadius = badgeLabel.bounds.size.width > MINIWIDTHANDHEIGHT ? 5: badgeLabel.bounds.size.width / 2;
}
