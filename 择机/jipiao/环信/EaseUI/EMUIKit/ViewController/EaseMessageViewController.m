...
//        NSString *tmp = _conversation.conversationId;
...
//                NSLog(@"收到订单号码是 tmp -- %@",tmp);
//                NSDictionary *dictTmp = @{@"num":tmp,};
//                NSNotification *notification =[NSNotification notificationWithName:Receivecustomerserviceordernumber object:nil userInfo:dictTmp];
//                [[NSNotificationCenter defaultCenter] postNotification:notification];
                __weak typeof(self) weakSelf = self;
                [[TWClient sharedInstance]orderInfoWithorderId:tmp success:^(id dataDict, NSString *msg, NSInteger code) {
//                    orderInfo *orderInfodata = [MTLJSONAdapter modelOfClass:[orderInfo class] fromJSONDictionary:dataDict error:nil];
//                    NSDictionary *ext = @{@"orderInfo":orderInfodata};
//                    message.ext = ext;
                        NSMutableDictionary *mudic = [dataDict mutableCopy];
                        [mudic setValue:@"yes" forKey:@"orderInfo"];
                        message.ext = mudic;
                    if ([weakSelf.conversation.conversationId isEqualToString:message.conversationId]) {
                        ...
                        [weakSelf _sendHasReadResponseForMessages:@[message]
                                                       isRead:NO];
                        
                        if ([weakSelf _shouldMarkMessageAsRead])
                        {
                            [weakSelf.conversation markMessageAsReadWithId:message.messageId];
                        }
                    }
                } failure:^(NSInteger errorCode, NSString *msg) {
//                    orderInfo *orderInfodata = [[orderInfo alloc]init];
//                    orderInfodata.relationName = @"张三";
//                    orderInfodata.relationMobile = @"17004965055";
//                    orderInfodata.totalPrice = [NSNumber numberWithInteger:545];
//                    orderInfodata.depDate = @"2016-10-8 星期六";
//                    orderInfodata.depTime = @"10:30";
//                    orderInfodata.arrTime = @"02:30";
//
//                    orderInfodata.depCityName = @"衡阳";
//                    orderInfodata.arrCityName = @"深圳";
//                    orderInfodata.depTerminal = @"上海虹桥T2";
//                    orderInfodata.arrTerminal = @"上海虹桥T3";
//                    orderInfodata.flightNo = @"H2908";
//                    orderInfodata.airLogo = @"http://avatar.csdn.net/A/1/2/1_nt_tian.jpg";
//                    
//                    
//                    orderInfodata.flyTime = @"3min";
//                    orderInfodata.stopFlag = @"1";
//                    orderInfodata.stopCityName = @"拉萨";
//                    orderInfodata.stopCityArrTime = @"11:33";
//                    orderInfodata.lunarDepDate = @"农历哦";
//                    orderInfodata.airLogo = @"http://avatar.csdn.net/A/1/2/1_nt_tian.jpg";
...
//                    orderInfodata.psgInfo = [NSMutableArray arrayWithObjects:datddd,datddd1, nil];
                    
//                    NSDictionary *ext = @{@"orderInfo":[orderInfodata dictionaryValue]};
//                    message.ext = ext;
...
//        //添加昵称和头像扩展
//        NSDictionary *data = @{
//                               HXAVATAR:[Ticket sharedInstance].userinfo.displayName,
//                               HXNICKNAME:[Ticket sharedInstance].userinfo.avatar,
//                               };
...
- (NSDictionary*)getUserInfoAttribute
{
    ...
    if ([Ticket sharedInstance].userinfo.displayName) {
        [visitor setObject:[Ticket sharedInstance].userinfo.displayName forKey:@"trueName"];
    }
    if ([Ticket sharedInstance].userinfo.phone) {
        [visitor setObject:[Ticket sharedInstance].userinfo.phone forKey:@"phone"];
    }
    if ([Ticket sharedInstance].userinfo.displayName) {
        [visitor setObject:[Ticket sharedInstance].userinfo.displayName forKey:@"userNickname"];
    }
    if ([Ticket sharedInstance].userinfo.avatar) {
        [visitor setObject:[Ticket sharedInstance].userinfo.avatar forKey:HXAVATAR];
    }
//    switch (_saleType) {
//        case ePreSaleType:
