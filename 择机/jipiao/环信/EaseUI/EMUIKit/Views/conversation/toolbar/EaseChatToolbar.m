...
- (void)setInputViewLeftItems:(NSArray *)inputViewLeftItems
{
    ...
    CGFloat oX = self.toolbarView.frame.size.width - self.horizontalPadding ;
//    CGFloat oX = self.horizontalPadding;
    ...
    for (id item in inputViewLeftItems) {
        if ([item isKindOfClass:[EaseChatToolbarItem class]]) {
            ...
            if (chatItem.button) {
                ...
                itemFrame.origin.x = oX - itemHeight;
    ...
    CGFloat value = self.horizontalPadding + itemHeight;
...
//    CGFloat oMaxX = self.toolbarView.frame.size.width - self.horizontalPadding;
    CGFloat itemHeight = self.toolbarView.frame.size.height - self.verticalPadding * 2;
    CGFloat oMaxX =self.horizontalPadding *2 + itemHeight*2;
    ...
    CGRect inputFrame = self.inputTextView.frame;
    CGFloat value = self.horizontalPadding *2 + itemHeight*2;
    inputFrame.size.width -= value;
    inputFrame.origin.x  += value;
    self.inputTextView.frame = inputFrame;
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    //转义回来
    NSMutableString *attStr = [[NSMutableString alloc] initWithString:textView.attributedText.string];
                                           ...
                                           usingBlock:^(id value, NSRange range, BOOL *stop)
     {
         if (value) {
             if ([value isKindOfClass:[EMTextAttachment class]]) {
                 ...
                 NSTextAttachment* attachment2 = (NSTextAttachment*)value;
                 NSArray *arrEmoji = [GlobalObject getEmojiArray];
                 for (int i = 0; i < arrEmoji.count; i ++)
                 {
                     UIImage *image = [UIImage imageNamed:arrEmoji[i][@"png"]];
                     if ([image isEqual:attachment2.image])
                     {
                         NSString *str = [NSString stringWithFormat:@"%@",arrEmoji[i][@"chs"]];
    ...
    if ([GlobalObject attributedStringWithTextYE:attStr]) {
         textView.attributedText = [GlobalObject attributedStringWithText:attStr];
    }
    NSLog(@"textView=%@",textView.text);
}
            ...
            if (chatText.length > 0) {
                ...
                if (chatText.length >= 2) {
                    ...
                    if ([EaseEmoji stringContainsEmoji:subStr]) {
                        length = 2;
                    }else if ([GlobalObject attributedStringWithTextYE:chatText]){
                        length = 6;
                    }
                }
...
{
    NSNotification *notification =[NSNotification notificationWithName:CallPhone object:nil userInfo:nil];
