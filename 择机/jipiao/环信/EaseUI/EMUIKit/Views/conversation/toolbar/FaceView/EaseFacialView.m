    ...
    if ([emotion isKindOfClass:[EaseEmotion class]]) {
        if (emotion.emotionType == EMEmotionGif) {
            ...
            if ([GlobalObject attributedStringWithTextYE:emotion.emotionThumbnail] ) {
                [_imageButton setAttributedTitle:[GlobalObject attributedStringWithText:emotion.emotionThumbnail] forState:UIControlStateNormal];
                [_imageButton setTitle:nil forState:UIControlStateNormal];
            }else{
                [_imageButton setTitle:emotion.emotionThumbnail forState:UIControlStateNormal];
                [_imageButton setAttributedTitle:nil forState:UIControlStateNormal];
            }
...
- (void)sendEmotion:(id)sender
{
    if (_delegate) {
        if ([_emotion isKindOfClass:[EaseEmotion class]]) {
            ...
            [_delegate didSendEmotion:nil];  //删除／明天继续
        }
    }
}
