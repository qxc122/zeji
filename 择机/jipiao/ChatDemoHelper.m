...
//                    NSDictionary *ext = @{@"orderInfo":dataDict};
...
//                    message.ext = ext;
                    ...
                    [weakSelf shoudaoDingdan:message boo:needShowNotification];
        ...
        if (ext) {
            kefuInfo *dataaa = [MTLJSONAdapter modelOfClass:[kefuInfo class] fromJSONDictionary:ext[@"weichat"][@"agent"] error:nil];
            [NSKeyedArchiver archiveRootObject:dataaa toFile:ZEJIKEFUInfoPATH];
...
- (void)shoudaoDingdan:(EMMessage *)message boo:(BOOL)needShowNotification {
    BOOL isRefreshCons = YES;
    if (needShowNotification) {
#if !TARGET_IPHONE_SIMULATOR
        UIApplicationState state = [[UIApplication sharedApplication] applicationState];
...
-(ConversationListController *)conversationListVC{
    if (!_conversationListVC) {
        _conversationListVC = [[ConversationListController alloc]init];
    }
